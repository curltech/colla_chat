...
import {WebrtcPeer} from '@/libs/transport/webrtc/webrtc-peer'
import {config} from '@/libs/conf'
import {logService} from '@/libs/biz/log'
//import {LRUCache} from 'js-lru'
const LRUCache = require('js-lru').LRUCache
...
export class WebrtcPeerPool {
  ...
  private _signalAction: any = undefined
  ...
  registSignalAction(signalAction: any) {
  ...
  async create(peerId: string, options: any, router: any): Promise<WebrtcPeer> {
    let webrtcPeers: WebrtcPeer[] = undefined
    if (webrtcPeerPool.webrtcPeers.find(peerId)) {
      webrtcPeers = webrtcPeerPool.webrtcPeers.get(peerId)
    }
    let webrtcPeer = new WebrtcPeer(peerId, undefined, true, options, router)
    if (!webrtcPeers) {
      webrtcPeers = []
  ...
  getAll(): WebrtcPeer[] {
    ...
    webrtcPeerPool.webrtcPeers.forEach((key: string, peers: any[]) => {
  ...
  async receive(peerId: string, connectPeerId: string, connectSessionId: string, data: any) {
    ...
    let webrtcPeer: WebrtcPeer = undefined
  ...
  async receiveData(event: any) {
    let {
      receiveHandler
    } = webrtcPeerPool.getProtocolHandler(config.appParams.chainProtocolId)
    if (receiveHandler) {
