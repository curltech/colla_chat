...
class FlickVideoPlayerController extends AbstractMediaPlayerController {
  ...
  PlayerStatus get status {
    FlickManager? current = this.current;
    if (current != null) {
      if (current.flickVideoManager!.isPlaying) {
        return PlayerStatus.playing;
      }
      if (current.flickVideoManager!.isBuffering) {
        return PlayerStatus.buffering;
      }
      if (current.flickVideoManager!.isVideoEnded) {
        return PlayerStatus.completed;
      }
      if (current.flickVideoManager!.isVideoInitialized) {
...
class _PlatformFlickVideoPlayerState extends State<PlatformFlickVideoPlayer> {
  ...
  Widget _buildPlaylist(BuildContext context) {
    List<FlickManager> playlist = widget.controller._flickManagers;
...
class _PlatformFlickControllerPanelState
  ...
  Widget _buildVolumeButton(BuildContext context) {
    var label = widget.controller.getVolume().toStringAsFixed(1);
    return Ink(
        child: InkWell(
      child: Row(children: [
  ...
  Widget _buildSpeedButton(BuildContext context) {
    var label = widget.controller.getSpeed().toStringAsFixed(1);
    return Ink(
        child: InkWell(
      child: Row(children: [
