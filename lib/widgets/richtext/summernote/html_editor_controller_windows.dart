...
class HtmlEditorController extends unsupported.HtmlEditorController {
  ...
  dynamic _evaluateJavascript({required source}) async {
    if (!kIsWeb) {
      ...
      var result = await editorController!.runJavaScriptReturningResult(source);
      return result;
    } else {
      throw Exception(
  ...
  void _createController() {
    ...
    editorController!.setJavaScriptMode(JavaScriptMode.unrestricted);
    editorController!.setBackgroundColor(Colors.white);
    editorController!.setNavigationDelegate(NavigationDelegate(
