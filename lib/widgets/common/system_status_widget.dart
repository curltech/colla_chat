...
class SystemStatusWidget extends StatelessWidget with DataTileMixin {
  final ValueNotifier<Battery?> battery = ValueNotifier<Battery?>(null);

  final ValueNotifier<BatteryState?> batteryState =
      ValueNotifier<BatteryState?>(null);

  final ValueNotifier<SystemStatusModel?> systemStatusModel =
      ValueNotifier<SystemStatusModel?>(null);
  ...
  Widget _buildMacSystemMonitorWidget() {
    return ValueListenableBuilder(
        valueListenable: systemStatusModel,
        builder: (context, value, _) {
          ...
          List<DataTile> batteryTiles = [
            DataTile(
                title: 'Battery',
                titleTail:
                    '${systemStatusModel?.batteryStatus?.currentCapacity}%'),
            DataTile(
                title: 'Health',
  ...
  Widget _buildNonMacSystemMonitorWidget() {
    return ValueListenableBuilder(
        valueListenable: batteryState,
        builder: (context, value, _) {
          List<DataTile> batteryTiles = [
            DataTile(
                title: 'batteryState',
                titleTail: '${batteryState.value?.name}'),
            DataTile(
                title: 'batteryLevel',
                titleTail: '${battery.value?.batteryLevel}'),
            DataTile(
                title: 'batteryLevel',
          ...
          List<DataTile> systemResourceTiles = [
            DataTile(
                title: 'cpuLoadAvg',
                titleTail: '${SystemResources.cpuLoadAvg()}'),
            DataTile(
                title: 'memUsage', titleTail: '${SystemResources.memUsage()}'),
          ];
