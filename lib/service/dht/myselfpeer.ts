...
class MyselfPeerService extends PeerEntityService {
  ...
  Future<Myself?> login(loginData) async  {
    ...
    if (code_ && mobile_) {
      ...
      mobile = PhoneNumberUtil.format(mobile_, code_);
    }
  ...
  /// 修改密码
    Future<Myself>  changePassword(String oldPassword, String newPassword) async{
    // 使用新密码重新加密所有密钥（包括expired记录）
    var myselfPeers = await findByPeerId(myself.myselfPeer.peerId);
    if (myselfPeers!=null && myselfPeers.isNotEmpty) {
      var currentDate = DateTime.now();
      for (var myselfPeer in myselfPeers) {
        var privateKey = null;
        var priv = null;
        try {
          privateKey = await cryptoGraphy.import(myselfPeer.privateKey, {password: oldPassword});
          priv = await cryptoGraphy.import(myselfPeer.peerPrivateKey, oldPassword);
        } catch (e) {
          logger.e(e);
          throw 'WrongPassword';
        }
        myselfPeer.privateKey = await cryptoGraphy.export(privateKey, newPassword);
        myselfPeer.peerPrivateKey = await cryptoGraphy.export(newPassword, 'libp2p-key');
