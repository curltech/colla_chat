...
class PeerConnectionWidget extends StatefulWidget with TileDataMixin {
  const PeerConnectionWidget({Key? key}) : super(key: key);
...
class _PeerConnectionWidgetState extends State<PeerConnectionWidget> {
  ...
  _open() async {
    ...
    try {
      ...
      //添加本地Candidate事件监听
      localConnection.onIceCandidate = _onLocalCandidate;
      //添加本地Ice连接状态事件监听
      localConnection.onIceConnectionState = _onLocalIceConnectionState;

      //添加本地流至本地连接
      localConnection.addStream(_localStream!);
      //设置本地禁音状态为false
      ...
      //添加远端Candidate事件监听
      remoteConnection.onIceCandidate = _onRemoteCandidate;
      //监听获取到远端视频流事件
      remoteConnection.onAddStream = _onRemoteAddStream;
      //添加远端Ice连接状态事件监听
      remoteConnection.onIceConnectionState = _onRemoteIceConnectionState;
