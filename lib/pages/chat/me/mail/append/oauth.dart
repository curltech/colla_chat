...
abstract class OauthClient {
  ...
  OauthClientId? get oauthClientId => keyService.oauth[incomingHostName];
...
class GmailOAuthClient extends OauthClient {
  ...
  Future<OauthToken> _authenticate(String email) async {
    ...
    HttpClient? httpClient =
        HttpClientPool.instance.get('https://oauth2.googleapis.com');
    final response = await httpClient?.send('/token', {
      'client_id': clientId,
      'redirect_uri': '$callbackUrlScheme:/',
...
class OutlookOAuthClient extends OauthClient {
  ...
  Future<OauthToken> _authenticate(String email) async {
    ...
    // Use this code to get an access token
    HttpClient? httpClient =
        HttpClientPool.instance.get('https://login.microsoftonline.com');
    final response = await httpClient!.send('/common/oauth2/v2.0/token', {
      'client_id': clientId,
      'redirect_uri': callbackUrlScheme,
      'grant_type': 'authorization_code',
      'code': code,
    ...
    return OauthToken.fromText(response.data!);
  }
